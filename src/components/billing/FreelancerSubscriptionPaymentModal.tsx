...
async function fetchPublishableKey(): Promise<string | null> {
  ...
  try {
    ...
    if (error) {
      console.error("[FreelancerSubscriptionPaymentModal] Error fetching Stripe key:", error);
      return null;
    }
...
function SubscriptionForm({ planName, amount, intentType, onSetupComplete, onSuccess, onError }: SubscriptionFormProps) {
  const { formatLocalPrice } = useLocalizedPlanPrice();
    ...
    try {
      if (intentType === "setup") {
        const { error, setupIntent } = await stripe.confirmSetup({
          elements,
          confirmParams: {
            return_url: `${window.location.origin}/earnings?subscription=setup`,
          },
          redirect: "if_required",
        });

        if (error) {
          console.error("[FreelancerSubscriptionPaymentModal] Setup error:", error);
      ...
      const { error, paymentIntent } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `${window.location.origin}/earnings?subscription=success`,
        },
        redirect: "if_required",
      });

      if (error) {
        console.error("[FreelancerSubscriptionPaymentModal] Payment error:", error);
        onError(error.message || "Erro ao processar pagamento");
      } else if (paymentIntent && (paymentIntent.status === "succeeded" || paymentIntent.status === "requires_capture")) {
        console.log("[FreelancerSubscriptionPaymentModal] Payment succeeded:", paymentIntent.id);
        onSuccess();
      }
    } catch (err) {
      console.error("[FreelancerSubscriptionPaymentModal] Error:", err);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="border rounded-lg p-4 bg-background">
        <PaymentElement
          id="freelancer-subscription-payment-element"
          options={{
            layout: {
              type: "accordion",
...
interface FreelancerSubscriptionPaymentModalProps {
  open: boolean;
...
export function FreelancerSubscriptionPaymentModal({
  open,
  onOpenChange,
  planType,
  planName,
  onSubscriptionConfirmed,
}: FreelancerSubscriptionPaymentModalProps) {
  ...
  const createSubscriptionIntent = async () => {
    try {
      ...
      const { data, error } = await supabase.functions.invoke("create-freelancer-subscription-intent", {
        body: { planType },
      });
      ...
      console.error("[FreelancerSubscriptionPaymentModal] Error creating intent:", err);
  ...
  const createSubscriptionFromPaymentMethod = async (paymentMethodId: string) => {
    try {
      ...
      console.error("[FreelancerSubscriptionPaymentModal] Error creating subscription:", err);
      setError(err instanceof Error ? err.message : "Erro ao criar assinatura");
    } finally {
      setCreating(false);
    }

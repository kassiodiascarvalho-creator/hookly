...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden lg:flex items-center gap-6">
              {navLinks.map((link) =>
                link.isAnchor ? (
                  <a key={link.label} href={link.to} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
                    {link.label}
                  </a>
                ) : (
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center max-w-6xl mx-auto">
            ...
            >
              <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-extrabold leading-[1.1] mb-6">
                {t("landing.hero.line1", "Contrate por")}
                <br />
                <span className="underline-gradient italic text-gradient-primary">
              ...
              <div className="flex flex-wrap items-center gap-4 mb-8">
                ...
                <Link to="/como-funciona">
                  <Button size="lg" variant="ghost" className="text-muted-foreground gap-2 text-lg">
              ...
              <div className="flex items-center gap-6 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-primary" /> {t("landing.hero.trust1", "100% Verificados")}</span>
                <span className="flex items-center gap-1.5"><Shield className="h-3.5 w-3.5 text-primary" /> {t("landing.hero.trust2", "Escrow Seguro")}</span>
                <span className="flex items-center gap-1.5"><Zap className="h-3.5 w-3.5 text-primary" /> {t("landing.hero.trust3", "Match em 24h")}</span>
              </div>
            </motion.div>

            {/* Right side — Mini search form */}
            <motion.div
              initial={{ opacity: 0, x: 30 }}
              animate={{ opacity: 1, x: 0 }}
                ...
                <div className="space-y-4 mb-6">
                  <div>
                    ...
                    <select className="w-full h-11 rounded-lg border border-border bg-background/80 px-4 text-sm text-foreground appearance-none cursor-pointer focus:border-primary/50 focus:outline-none transition-colors">
                      ...
                      <option>Redator/Copywriter</option>
      ...
      <section className="py-24 relative">
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-4xl mx-auto relative">
            ...
            {[
              { icon: FileText, step: "post", time: t("landing.howItWorks.time1", "⏱️ 3 minutos") },
              { icon: Users, step: "receive", time: t("landing.howItWorks.time2", "⏱️ 24 horas") },
              { icon: Sparkles, step: "hire", time: t("landing.howItWorks.time3", "✅ Entregáveis aprovados") },
            ].map((item, i) => (
              <motion.div
                key={item.step}
              ...
              >
                <div className={`flex-1 ${i % 2 === 0 ? "md:text-right" : "md:text-left"}`}>
                  <div className="surface-card-hover p-8">
                    <span className="text-xs text-primary font-semibold uppercase tracking-wider">
                      {t("howItWorksPage.step", "Passo")} {i + 1}
                    </span>
                    <h3 className="font-display text-xl font-semibold mt-2 mb-3">{t(`howItWorks.steps.${item.step}.title`)}</h3>
                    <p className="text-muted-foreground text-sm leading-relaxed mb-3">{t(`howItWorks.steps.${item.step}.description`)}</p>
                    <span className="text-xs text-primary/80">{item.time}</span>
                  </div>
                </div>

                {/* Center dot */}
                <div className="hidden md:flex flex-shrink-0 w-14 h-14 rounded-full bg-primary/20 items-center justify-center border-4 border-background z-10">
      ...
      <section className="py-24 relative">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {/* Starter */}
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="surface-card p-8">
              ...
              <ul className="space-y-3 mb-8">
                {(t("pricing.starter.features", { returnObjects: true }) as string[]).map((feature, i) => (
                  <li key={i} className="flex items-center gap-3 text-sm text-muted-foreground">
                    <Check className="h-4 w-4 text-primary shrink-0" /> {feature}
                  </li>
                ))}
              </ul>
              <Link to="/auth">
                <Button variant="outline" className="w-full border-border hover:bg-accent">{t("pricing.starter.cta")}</Button>
              </Link>
            </motion.div>

            {/* Business — with border glow */}
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="surface-card p-8 relative border-glow">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <span className="px-4 py-1 rounded-full text-xs font-semibold bg-primary text-primary-foreground">{t("pricing.business.popular")}</span>
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Accordion type="single" collapsible className="w-full space-y-3">
              {(t("faq.items", { returnObjects: true }) as Array<{ question: string; answer: string }>).map((item, i) => (
                <AccordionItem key={i} value={`item-${i}`} className="surface-card px-6 border border-border rounded-xl overflow-hidden">
                  <AccordionTrigger className="text-left hover:no-underline py-5 text-foreground">{item.question}</AccordionTrigger>
      ...
      {/* ─── SECTION 12: CTA FINAL — Email inline ─── */}
      <section className="py-24 relative">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-2xl mx-auto">
            <div className="surface-card p-10 md:p-14 text-center border-glow">
              <h2 className="font-display text-3xl md:text-4xl font-bold mb-4 text-gradient-primary">
                {t("landing.ctaFinal.title", "Seu próximo projeto merece talentos VERIFICADOS.")}
              </h2>
              ...
              <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto mb-6">
                ...
                />
                <Button className="btn-gradient gap-2 whitespace-nowrap" onClick={handleCtaEmail}>
                  <Rocket className="h-4 w-4" />
              ...
              <div className="flex flex-wrap justify-center gap-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5"><Check className="h-3 w-3 text-primary" /> {t("landing.ctaFinal.noCreditCard", "Sem cartão de crédito")}</span>
                <span className="flex items-center gap-1.5"><Check className="h-3 w-3 text-primary" /> {t("landing.ctaFinal.quickSetup", "Setup em 2 minutos")}</span>
                <span className="flex items-center gap-1.5"><Check className="h-3 w-3 text-primary" /> {t("landing.ctaFinal.cancelAnytime", "Cancele quando quiser")}</span>
              </div>
            </div>
          </motion.div>
        </div>
                ...
                {socialLinks && socialLinks.length > 0 ? (
                  socialLinks.map((link) => (
                    ...
                    >
                      ...
                      {link.icon === 'youtube' && (
                        <svg className="h-4 w-4 text-muted-foreground" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                      )}
                      {link.icon === 'tiktok' && (
                        <svg className="h-4 w-4 text-muted-foreground" fill="currentColor" viewBox="0 0 24 24"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/></svg>
                      )}
                    </a>
                  ))
                ) : (

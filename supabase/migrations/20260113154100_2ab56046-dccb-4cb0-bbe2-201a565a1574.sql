-- Update request_withdrawal function to work with minor units (cents)
-- The p_amount parameter now receives minor units (e.g., 200 for R$2.00)
...
DECLARE
  ...
  v_currency TEXT;
BEGIN
  -- p_amount is now in MINOR UNITS (cents)
  -- Validate it's a positive integer-like value
  IF p_amount <= 0 THEN
    RAISE EXCEPTION 'Withdrawal amount must be positive';
  END IF;

  -- Get current earnings (stored in minor units)
  ...
  IF v_current_earnings IS NULL OR v_current_earnings < p_amount THEN
    RAISE EXCEPTION 'Insufficient earnings balance. Available: %, Requested: %', COALESCE(v_current_earnings, 0), p_amount;
  END IF;
  ...
  -- Hold earnings (subtract from available) - both in minor units
  ...
  -- Create withdrawal request with amount in minor units
  ...
  -- Record transaction with amount in minor units
